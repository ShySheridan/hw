package server_main.utils;

import common_main.Coordinates;
import common_main.LabWork;
import common_main.Location;
import common_main.Person;
...
public class LabReader {
    ...
    public LabReader(Scanner scanner) {
    ...
    private Person readPerson() {
        ...
        Integer height = readInteger( value -> value > 0, "Рост должен быть больше 0");
    ...
    private Integer readInteger(java.util.function.Predicate<Integer> validator, String errorMessage) {
        while (true) {
            String input = scanner.nextLine().trim();
    ...
    private Long readLong() {
        while (true) {
            ...
            try {
                ...
                System.out.println(e.getMessage());
                throw new NumberFormatException("Некорректное целое число");
            }
        }
    }

    public LabWork readLab() {
